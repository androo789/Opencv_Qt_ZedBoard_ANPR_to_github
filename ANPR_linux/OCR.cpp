...
CharSegment::CharSegment(Mat i, Rect p){
    img=i;
    pos=p;
    cout<<i<<"\n==zhe  li  i  shi   mat  leixing      yishang ====================\n";
}
...
Mat OCR::preprocessChar(Mat in){//???????
   ...
   Mat warpImage(m, m, in.type());//??????????? ?? ???
     ...
     imwrite("/mnt/OCRtemp1.bmp",in);
      imwrite("/mnt/OCRtemp2.bmp",transformMat);
    warpAffine(in, warpImage, transformMat, warpImage.size(), INTER_LINEAR, BORDER_CONSTANT, Scalar(0) );
//????

    imwrite("/mnt/OCRtemp3.bmp",warpImage);
    Mat out;
    resize(warpImage, out, Size(charSize, charSize) );//20????

//????
    imwrite("/mnt/OCRtemp4.bmp",out);
    return out;
}
...
vector<CharSegment> OCR::segment(Plate plate){//?????  ???
    ...
    img_threshold.copyTo(result);
     ...
     imwrite("/mnt/OCRtemp5.bmp",img_threshold);
  ...
  imwrite("/mnt/OCRtemp6.bmp",result);
    ...
    while (itc!=contours.end()) {
        ...
        rectangle(result, mr, Scalar(0,255,0));//??? ?red
        //Crop image????
        Mat auxRoi(img_threshold, mr);//???? ?????????? ????????  ??????

         imwrite("/mnt/OCRtemp7.bmp",auxRoi);

        if(verifySizes(auxRoi)){//??????  ????
            imwrite("/mnt/OCRtemp8.bmp",auxRoi);
                                                cout << auxRoi << "\n=*******before   auxRoi yi shang=*************===\n";
            auxRoi=preprocessChar(auxRoi);
                                            cout << auxRoi << "\n=*******after   auxRoi yi shang=*************===\n";
             imwrite("/mnt/OCRtemp9.bmp",auxRoi);
...
Mat OCR::features(Mat in, int sizeData){//????
    ...
    {
        out.at<float>(j)=vhist.at<float>(i);//at??????????
        j++;
    }
...
string OCR::run(Plate *input){//???????? ???
    ...
    qDebug()<<segments.size()<<"segments.size()  number is";

                                         cout<<segments[0].img<<"\n============segments[0].img  yi  shang ============================\n";
